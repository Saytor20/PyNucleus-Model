...
class PerformanceAnalyzer:
    ...
    def analyze_document_processing(self, processed_dir="processed_txt_files"):
        """
    ...
    def analyze_chunking(self, chunked_data_path="Chuncked_Data/chunked_data_stats.json"):
        """
        ...
        if os.path.exists(chunked_data_path):
            try:
                ...
                if chunk_lengths:
                    metrics["total_chunks"] = data.get("total_chunks", len(chunk_lengths))
    ...
    def analyze_vector_store(self, vec_dir="faiss_store"):
        ...
        if os.path.exists(vec_dir):
            ...
            total_size = sum(os.path.getsize(os.path.join(vec_dir, f)) for f in os.listdir(vec_dir) if os.path.isfile(os.path.join(vec_dir, f)))
            ...
            try:
                ...
                print("Warning: 'faiss' library not found. Cannot read document count from index.")
            except Exception as e:
    ...
    def analyze_search_performance(self, log_dir="vectordb_outputs"):
        ...
        if os.path.exists(log_dir):
            log_files = [f for f in os.listdir(log_dir) if f.startswith("faiss_analysis_")]
            if log_files:
                ...
                try:
                    with open(os.path.join(log_dir, latest_log), 'r') as f:
                        ...
                        if metrics["total_queries"] > 0:
                            metrics["recall_rate"] = metrics["successful_queries"] / metrics["total_queries"]
                except Exception as e:
