...
def check_dwsim_availability():
    ...
    try:
        ...
        return False, "pythonnet (clr) not available - install with: pip install pythonnet"
...
def get_dwsim_status():
    ...
    status = {
        ...
        'dwsim_files_count': len(list(Path("examples").glob("*.dwsim"))) if Path("examples").exists() else 0
    }
    return status
