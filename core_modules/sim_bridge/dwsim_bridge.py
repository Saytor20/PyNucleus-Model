...
class DWSIMBridge:
    ...
    def _load_dwsim_dlls(self) -> None:
        ...
        if not loaded_dlls:
            raise RuntimeError(f"Failed to load any DLL files. Errors: {failed_dlls}")
    ...
    def export_stream_data(
        ...
        *,
        ...
        if self._flowsheet is None:
            raise RuntimeError(
                "No flowsheet is loaded – run new_case() or load_case() first."
            )
        ...
        for obj in self._flowsheet.SimulationObjects.Values:
            ...
            rows.append(
                dict(
                    ...
                    temperature_K=obj.PhaseProperties[
                        1
                    ].Temperature.GetValueOrDefault(),
                    pressure_kPa=obj.PhaseProperties[1].Pressure.GetValueOrDefault()
                    / 1000.0,
                )
            )
