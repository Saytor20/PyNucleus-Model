...
def _load_docs(json_path: str, log_func) -> List[Document]:
    """Load documents from JSON file."""
    try:
        with open(json_path, "r", encoding="utf-8") as f:
            data = json.load(f)
        log_func(f"Loaded {len(data)} documents from {json_path}")
        ...
        log_func(f"⚠️  {e} – falling back to 3 dummy docs.")
        dummy = [
            ("Modular chemical plants reduce construction time.", {"source": "dummy_1"}),
            ("Scalability is a key advantage of modular design.", {"source": "dummy_2"}),
...
class FAISSDBManager:
    def __init__(self, store_dir: str = "vector_db"):
        ...
        # Set up logging
        self.log_dir = Path("chunk_reports")
    ...
    def evaluate(self, ground_truth: Dict[str, str], k: int = 3) -> None:
        ...
        for query, expected_id in ground_truth.items():
            ...
            self.log(f"Q: {query[:40]:<40} {'✓' if found else '✗'}   top-score={top_score:.4f}")
